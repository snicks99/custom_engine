...
#            define MMAP_DEFAULT( s ) ( ( dev_zero_fd < 0 ) ? ( dev_zero_fd = open( "/dev/zero", O_RDWR ),                \
...
#   ifdef MALLOC_ZERO_BYTES_NULL
    if ( bytes == 0 )
        return 0;
#   endif /* MALLOC_ZERO_BYTES_NULL */
...
#   if  FOOTERS
            if ( mem != 0 )
            {
                const size_t cz = chunksize( mem2chunk( mem ) );
                const int diff = cz - oldsize;
...
#   if FOOTERS
                const size_t cz = chunksize( newp );
                const int diff = cz - oldsize;
