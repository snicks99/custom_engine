    ...
    std::string IniKeyToEnvVarKey(std::string const& key)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            if (!isEnd)
            {
                ...
                currIsNumeric = Util::isNumeric(curr);
                nextIsNumeric = Util::isNumeric(str[i + 1]);
...
T ConfigMgr::GetValueDefault(std::string const& name, T def, bool quiet) const
{
    ...
    catch (bpt::ptree_bad_path const&)
    {
        ...
        if (envVar)
        {
            ...
            if (!castedVar)
            {
                DBL_ERROR("Bad value defined for name %s in environment variables, going to use default instead", name.c_str());
                return def;
            }

            if (!quiet)
                DBL_WARNING("Missing name %s in config file %s, recovered with environment '%s' value.", name.c_str(), _filename.c_str(), envVar->c_str());

            return *castedVar;
        }
        else if (!quiet)
        {
            DBL_WARNING("Missing name %s in config file %s, add \"%s = %s\" to this file",
                name.c_str(), _filename.c_str(), name.c_str(), std::to_string(def).c_str());
        }
    }
    catch (bpt::ptree_bad_data const&)
    {
        DBL_ERROR("Bad value defined for name %s in config file %s, going to use %s instead",
