...
//==========================================
#include "Debug/WheatyExceptionReport.h"
#include "Common/Types.h"
#include "Debug/Errors.h"
#include "Git/GitRevision.h"
#include <algorithm>
#include <ehdata.h>
#include <rttidata.h>
#include <tlhelp32.h>
    ...
    PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    sprintf( m_szDumpFileName, "%s\\%s_%s_[%u-%u_%u-%u-%u].dmp", crash_folder_path, GitRevision::GetHash(), pos, systime.wDay, systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond );

    _stprintf( m_szLogFileName, _T("%s\\%s_%s_[%u-%u_%u-%u-%u].txt"), crash_folder_path, GitRevision::GetHash(), pos, systime.wDay, systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond );
    ...
    if ( m_hDumpFile )
    {
        ...
        MiniDumpWriteDump( GetCurrentProcess(), GetCurrentProcessId(), m_hDumpFile, MiniDumpWithIndirectlyReferencedMemory, &info, &additionalStreamInfo, nullptr );
...
BOOL WheatyExceptionReport::_GetProcessorName( TCHAR* sProcessorName, DWORD maxcount )
{
    ...
    lRet           = ::RegQueryValueEx( hKey, _T("ProcessorNameString"), nullptr, nullptr, (LPBYTE)szTmp, &cntBytes );
    if ( lRet != ERROR_SUCCESS )
        return FALSE;
    ::RegCloseKey( hKey );
    sProcessorName[0] = '\0';
    ...
    switch ( osvi.dwPlatformId )
    {
        ...
        case VER_PLATFORM_WIN32_NT:
        {
            ...
            if ( osvi.dwMajorVersion == 4 && _tcsicmp( szCSDVersion, _T("Service Pack 6") ) == 0 )
            {
                ...
                if ( lRet == ERROR_SUCCESS )
                {
                    _stprintf( wszTmp, _T("Service Pack 6a (Version %d.%d, Build %d)"), osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF );
                    _tcsncat( szVersion, wszTmp, cntMax );
                }
                else    // Windows NT 4.0 prior to SP6a
                {
                    _stprintf( wszTmp, _T("%s (Version %d.%d, Build %d)"), szCSDVersion, osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF );
                    _tcsncat( szVersion, wszTmp, cntMax );
                }
                ::RegCloseKey( hKey );
            }
...
BOOL WheatyExceptionReport::_GetWindowsVersionFromWMI( TCHAR* szVersion, DWORD cntMax )
{
    ...
    std::shared_ptr<IEnumWbemClassObject> queryResult = [svc]() -> std::shared_ptr<IEnumWbemClassObject>
    {
        ...
        return { tmp, []( IEnumWbemClassObject* ptr )
                 { if (ptr) ptr->Release(); } };
    }();
    ...
    PEXCEPTION_POINTERS pExceptionInfo )
{
    __try
    {
        ...
        if ( pExceptionRecord->ExceptionCode == 0xE06D7363 && pExceptionRecord->NumberParameters >= 2 )
        {
            ...
            auto resolveExceptionRVA = []( void const* input ) -> void const*
            { return input; };
#endif
...
#define EXCEPTION( x )  \
    ...
    switch ( dwCode )
    {
        ...
        case 0xE06D7363:
            return _T("Unhandled C++ exception");
    }
    ...
    DWORD_PTR& offset )
{
          ...
          i++, pSection++ )
    {
        ...
        DWORD_PTR sectionEnd   = sectionStart + DWORD_PTR( std::max( pSection->SizeOfRawData, pSection->Misc.VirtualSize ) );
    ...
    switch ( typeTag )
    {
            ...
            if ( SymGetTypeInfo( m_hProcess, modBase, dwTypeIndex, TI_GET_TYPEID, &innerTypeID ) )
            {
                ...
                switch ( innerTypeTag )
                {
                    case SymTagUDT:
                        if ( symbolDetails.size() >= WER_MAX_NESTING_LEVEL )
                            logChildren = false;
                        DumpTypeIndex( modBase, innerTypeID, offset, bHandled, symbolDetails.top().Name.c_str(), "", false, logChildren );
                        break;
                    case SymTagPointerType:
                        if ( Name != nullptr && Name[0] != '\0' )
                            symbolDetails.top().Name = Name;
        ...
        case SymTagArrayType:
            if ( SymGetTypeInfo( m_hProcess, modBase, dwTypeIndex, TI_GET_TYPEID, &innerTypeID ) )
            {
                ...
                DumpTypeIndex( modBase, innerTypeID, offset, bHandled, Name, "", false, false );
        ...
        switch ( basicType )
        {
            ...
            default:
                ...
                else if ( length == 8 )
                {
                    if ( basicType == btFloat )
                    {
                        pszCurrBuffer += sprintf( pszCurrBuffer, "%f", *(double*)pAddress );
                    }
                    else
