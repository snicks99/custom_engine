...
bool8 File::Open( const char8* filePath, uint32 mode, Disposition disposition )
{
    ...
    if ( hFl == nullptr )
    {
        MGE_ERROR( false, "Couldn't open file %s (%u - %u)", filePath, mode, disposition );
        return false;
    }
...
{
    MGE_ASSERT( mOpenMode & IO_OPEN_READ, "File %s not opened with READ flag", mFilePath.c_str() );
    MGE_ASSERT( mFileHandle );
...
{
    MGE_ASSERT( mOpenMode & IO_OPEN_WRITE, "File %s not opened with WRITE flag", mFilePath.c_str() );
    MGE_ASSERT( mFileHandle );
