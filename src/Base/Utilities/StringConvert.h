...
namespace Trinity::Impl::StringConvertImpl
{
    ...
    struct For<T, std::enable_if_t<std::is_integral_v<T> && !std::is_same_v<T, bool>>>
    {
        static Optional<T> FromString(std::string_view str, int base = 10)
        {
            if (base == 0)
            {
                if (Util::StringEqualI(str.substr(0, 2), "0x"))
                {
                    base = 16;
                    str.remove_prefix(2);
                }
                else if ( Util::StringEqualI( str.substr( 0, 2 ), "0b" ) )
                {
                    base = 2;
                    str.remove_prefix(2);
                }
    ...
    */
    ...
    struct For<bool, void>
    {
        static Optional<bool> FromString(std::string_view str, int strict = 0) /* this is int to match the signature for "proper" integral types */
        {
            ...
            {
                if ( ( str == "1" ) || Util::StringEqualI( str, "y" ) || Util::StringEqualI( str, "on" ) || Util::StringEqualI( str, "yes" ) || Util::StringEqualI( str, "true" ) )
                    return true;
                if ( ( str == "0" ) || Util::StringEqualI( str, "n" ) || Util::StringEqualI( str, "off" ) || Util::StringEqualI( str, "no" ) || Util::StringEqualI( str, "false" ) )
                    return false;
                return std::nullopt;
            }
        }
