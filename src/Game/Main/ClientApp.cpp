...
bool ClientApp::RegisterFileSystem()
{
    ...
    {
        DBL_ERROR( false, "Can't get current directory" );
        return false;
    }
...
bool ClientApp::CreateSubSystem( int32 width, int32 height, bool zBuffer )
{
    ...
    // Framework Layer
    DBL_ASSERT( Application::CreateSubSystem( width, height, zBuffer ) );
    ...
    {
        DBL_ERROR( false, "Can't create the render camera" );
        return false;
    }
    ...
    if ( mCharset == nullptr )
    {
        DBL_ERROR( false, "Can't create RtCharset" );
        return false;
    }

    if ( mSceneMgr.Create() == false )
    {
        DBL_ERROR( false, "Can't create SceneMgr" );
        return false;
    }
